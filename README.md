...
프로젝트 화면에서 Git repo와 투입 에이전트, 마스터, 라우팅 방식을 정합니다. 자동 라우팅은 요청을 `debugger`, `frontend`, `backend`, `architect`, `qa` 같은 작업 모드에 연결하며 PM→개발→디자인→QA 같은 고정 순서를 강제하지 않습니다. 필요하면 마스터 직접 처리나 사용자가 지정한 순서를 선택할 수 있습니다. 왼쪽 사이드바 최상단에서 repo를 바꾸면 캔버스·작업 선반·통계가 해당 repo 기준으로 전환됩니다. 같은 repo 경로는 기본적으로 한 에이전트만 쓰도록 직렬화되며, 실제 병렬 실행은 에이전트별 Git worktree를 별도 경로로 준비하는 방식을 권장합니다.
